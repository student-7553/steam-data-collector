...
const fromDate = new Date("2022-04-08");
const toDate = addYears(fromDate, 1);

const tableName = "indie_2022_4__2023_4";
...
async function run() {
  ...
  const indieSteamGames = steamGames.filter((steamGame) => {
    ...
    return mappedTags[steamGame.steam_appid].includes("Indie");
  });
  await mapData(indieSteamGames, mappedTags);
}
